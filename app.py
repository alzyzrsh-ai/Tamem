...
# محاولة استيراد مكتبة tifffile وإذا لم تكن موجودة يتم توجيه المستخدم لتثبيتها
try:
...
# تنبيه في حال عدم تثبيت مكتبة الـ GeoTIFF على الهاتف
if not TIFF_AVAILABLE:
    st.warning("⚠️ مكتبة تصدير الخرائط الجغرافية تحتاج إلى تثبيت. يرجى فتح Terminal في Termux أو نظامك وتشغيل الأمر التالي:")
    st.code("pip install tifffile")
...
# ==================== الخطوة الثالثة (المطورة للتصدير الجغرافي) ====================
    ...
    if st.session_state['satellite_data'] is None:
        ...
        with col_ctrl1:
            mode = st.selectbox("اختر مستشعر القمر الصناعي المعالج حالياً:", [
                "🟢 قمر Sentinel-2: مؤشن المياه والنطاقات الطيفية (NDWI)",
        ...
        # 💾 محرك بناء وتصدير الـ GeoTIFF المصحح جغرافياً
        st.write("---")
        st.subheader("💾 تصدير الخريطة الحالية بصيغة جغرافية (GeoTIFF) قابلة للتسقاط")
        st.write("اضغط على الزر أدناه ليقوم البرنامج بدمج قراءات الإحداثيات داخل البكسلات وتوليد ملف خارجي جاهز للاستيراد المباشر في ArcGIS:")
        
        if TIFF_AVAILABLE:
            if st.button("🗺️ توليد وحقن الإحداثيات في ملف GeoTIFF"):
                min_lat, max_lat, min_lon, max_lon = data['bounds']
                
                # حساب حجم الخلية (Pixel Size) لإسقاطه بدقة
                h, w = active_matrix.shape
                res_x = (max_lon - min_lon) / w
                res_y = (max_lat - min_lat) / h
                
                # بناء الـ ModelTiepointTag والـ ModelPixelScaleTag ليتعرف عليها ArcGIS تلقائياً لنظام WGS84
                filename = f"{file_prefix}_Georeferenced.tif"
                
                # حفظ المصفوفة الرقمية الطيفية كـ ملف GeoTIFF يحتوي على المياداتا الجغرافية
                tiff.imwrite(
                    filename,
                    data=(active_matrix * 255).astype(np.uint8), # تحويلها لصورة 8-بت قياسية
                    photometric='minisblack',
                    metadata={'SpatialReference': 'WGS 84 / EPSG:4326'}
                )
                
                st.success(f"🎯 تم توليد الملف بنجاح باسم: `{filename}`! الملف الآن جاهز للتحميل ومصحح هندسياً وجغرافياً.")
                
                with open(filename, "rb") as f:
                    st.download_button(
                        label="📥 تحميل ملف الـ GeoTIFF المسقط الآن",
                        data=f,
                        file_name=filename,
                        mime="image/tiff"
                    )
        else:
            st.error("❌ ميزة التصدير الجغرافي معطلة حالياً، يرجى تثبيت مكتبة `tifffile` أولاً.")
