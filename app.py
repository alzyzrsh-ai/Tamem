...
# ==========================================
# 6️⃣ المرحلة السادسة: المسح الجيوفيزيائي (VES)
# ==========================================
elif "6️⃣" in step:
    st.header("⚡ قسم الجيوفيزياء - السبر الكهربائي العمودي (VES)")
    st.markdown("---")
    ...
    # ----------------------------------------------------
    # ج) معالجة الصورة عبر الذكاء الاصطناعي عند الطلب
    # ----------------------------------------------------
    if image_source is not None:
        ...
        if st.sidebar.button("🔍 قراءة وتفريغ الجدول من الصورة"):
            if not api_key:
                st.sidebar.error("⚠️ يرجى أدخال مفتاح API أولاً!")
            else:
                try:
                    with st.spinner("جاري قراءة خط اليد واستخراج البيانات..."):
                        ...
                        prompt = """
                        Extract the table data from this geophysics field sheet (VES Schlumberger).
                        Columns to extract:
                        - AB/2
                        - MN/2
                        - Current (I in mA)
                        ...
                        Return ONLY valid raw JSON array of objects with keys: "AB/2 (m)", "MN/2 (m)", "I (mA)", "V (mV)".
    ...
    # ----------------------------------------------------
    # د) عرض الجدول وحساب النتائج والرسم البياني
    # ----------------------------------------------------
    st.subheader("1. إدخال وتعديل قراءات المسح")
    edited_df = st.data_editor(st.session_state["ves_data"], num_rows="dynamic", key="ves_editor")
    
    # الحسابات الهندسية
    ab2 = edited_df["AB/2 (m)"]
    mn2 = edited_df["MN/2 (m)"].replace(0, np.nan)
    I_val = edited_df["I (mA)"].replace(0, np.nan)
    V_val = edited_df["V (mV)"]
    ...
    calc_df["R (Ohm)"] = np.round(R, 3)
    calc_df["K (m)"] = np.round(K, 2)
    calc_df["Apparent Resistivity (Ohm.m)"] = np.round(rho_a, 2)

    st.subheader("2. الجدول الحسابي للمقاومية الظاهرية")
    st.dataframe(calc_df, use_container_width=True)

    # تصدير إكسل
    excel_buffer = io.BytesIO()
    with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
        calc_df.to_excel(writer, sheet_name='VES_Results', index=False)

    st.download_button(
        label="📥 تحميل النتائج كملف Excel",
        data=excel_buffer.getvalue(),
        file_name="VES_Geophysics_Data.xlsx",
        mime="application/vnd.ms-excel"
    )

    st.markdown("---")
    st.subheader("3. منحنى السبر الكهربائي (VES Log-Log Curve)")
    
    if not calc_df["Apparent Resistivity (Ohm.m)"].isna().all():
        fig = px.line(
            calc_df, 
            x="AB/2 (m)", 
            y="Apparent Resistivity (Ohm.m)", 
            markers=True, log_x=True, log_y=True,
            title="منحنى المقاومية الظاهرية vs العمق (Schlumberger Array)",
            labels={"AB/2 (m)": "AB/2 (متر)", "Apparent Resistivity (Ohm.m)": "المقاومية الظاهرية (أوم.متر)"}
        )
        fig.update_traces(line_color="#112d4e", marker=dict(size=8, color="#e84545"))
