...
# ==================== الخطوة الثالثة ====================
    ...
    if st.session_state['satellite_data'] is None:
        ...
        st.write("---")
        st.write("💾 **لوحة الحفظ والتسمية اليدوية للملف الجغرافي:**")
        # التسمية اليدوية التي يفضلها المهندس
        user_filename = st.text_input("اكتب اسم الملف الذي تريده هنا (مثال: Al_Bayda_Sentinel2):", value="My_Project_Map")
        
        if st.button("🗺️ توليد وحفظ ملف GeoTIFF بالاسم المكتوب"):
            if not user_filename.strip():
                st.error("⚠️ يرجى كتابة اسم صالح للملف أولاً.")
            else:
                final_name = f"{user_filename}.tif"
                geotiff_bytes = export_pure_geotiff(active_matrix, data['bounds'])
                with open(final_name, "wb") as f: f.write(geotiff_bytes)
                st.success(f"✅ تم الحفظ يدوياً بنجاح! اسم ملفك الحالي الجاهز للآرك ماب: `{final_name}`")
...
# ==================== الخطوة الرابعة ====================
    ...
    if st.session_state['satellite_data'] is None:
        ...
        if filter_zone == "كل النطاقات":
            ...
            target_df = df_structural[df_structural['التصنيف البنيوي للطبقة'] == filter_zone]
            
        st.dataframe(target_df.head(200))
        
        st.write("---")
        st.write("💾 **تسمية وحفظ ملف الخصائص يدوياً:**")
        user_csv_name = st.text_input("اكتب اسم ملف الجدول الذي تريده (مثال: Stratigraphy_Report):", value="Structural_Stratigraphic_Properties")
        
        if not target_df.empty:
            csv_export = target_df.to_csv(index=False).encode('utf-8')
            st.download_button(
                label=f"📥 اضغط هنا لحفظ جدول ({filter_zone}) يدوياً بصيغة Excel/CSV",
                data=csv_export,
...
# ==================== الخطوة الخامسة ====================
    ...
    if st.session_state['satellite_data'] is None:
        ...
        st.write("يقوم المحرك الآن بتحليل تدرج المصفوفات الرادارية وعزل الحواف الخطية الطويلة التي تمثل كسوراً تكتونية تحت سطحية:")
        ...
        if not df_lines.empty:
            ...
            user_line_name = st.text_input("اكتب اسم ملف الصدوع المتجهي (مثال: Structural_Faults):", value="Automated_Fractures_Lines")
            
            csv_lines = df_lines.to_csv(index=False).encode('utf-8')
            st.download_button(
                label="📥 اضغط هنا لحفظ متجهات الكسور يدوياً وتصديرها كملف CSV",
                data=csv_lines,
                file_name=f"{user_line_name}.csv",
                mime="text/csv"
            )
            st.info("💡 **تذكير هندسي للـ ArcMap:** بعد الحفظ اليدوي، افتح الملف في ArcMap باستخدام أداة **(XY To Line)** ليرسم لك شبكة الكسور تلقائياً.")
