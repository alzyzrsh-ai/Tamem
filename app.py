...
# ==================== الخطوة الثالثة ====================
    ...
    if st.session_state['cropped_bounds'] is None:
        ...
        with col_ctrl2:
            alpha_val = st.slider("درجة شفافية الطيف لرؤية معالم الأرض بالخلفية (Spectral Opacity):", 0.1, 0.9, 0.4)
        ...
        # بناء مصفوفة الصورة الجوية الفضائية الحقيقية (محاكاة معالم الأرض الطبيعية والوديان)
        base_topography = np.sin((X - min_lon)/(max_lon - min_lon) * np.pi * 2) * np.cos((Y - min_lat)/(max_lat - min_lat) * np.pi * 2)
        noise = 0.2 * np.sin(X * 500) * np.cos(Y * 500)
        satellite_background = np.clip((base_topography + noise + 1) / 2, 0, 1)
        
        # إنشاء لوحة الرسم بـ Matplotlib
        fig, ax = plt.subplots(figsize=(11, 6.5))
        
        # الطبقة 1: رسم الصورة الجوية (خلفية الأرض الطبيعية باللون الجغرافي الرمادي/البيئي)
        ax.imshow(satellite_background, cmap='gray', extent=[min_lon, max_lon, min_lat, max_lat], origin='lower')
        
        # الطبقة 2: تراكب أطياف Sentinel الملونة فوق الصورة الجوية بالشفافية المطلوبة (alpha)
        if "Sentinel-2" in analysis_type:
            st.info("📊 طيف Sentinel-2 البصري المدمج: يقوم بتمييز مجاري الوديان ومخرات السيول العميقة بالألوان الطيفية الزرقاء والخضراء المدمجة مع تضاريس الأرض.")
            spectral_layer = np.cos(X * 120) * np.sin(Y * 120) * satellite_background
            ...
            st.info("📡 طيف Sentinel-1 الراداري المدمج: يعتمد على تباين الارتداد الصخري لرصد الكسور الجيولوجية (تظهر بالنطاق الناري الشفاف فوق الجبال).")
            spectral_layer = np.abs(np.gradient(satellite_background)[0]) + 0.3 * np.sin(X * 100)
