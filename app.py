...
# ==================== الخطوة الثالثة (تم حل مشكلة الـ TIF هنا) ====================
    ...
    if st.session_state['satellite_data'] is None:
        ...
        # صندوق النص لكتابة الاسم الجغرافي يدوياً
        ...
        if user_filename.strip():
            ...
            # توليد مصفوفة البايتات لملف الجيوتيف القياسي
            geotiff_bytes = export_pure_geotiff(active_matrix, data['bounds'])
            
            # زر التحميل الفوري الذي يسحب الملف لقرص الهاتف مباشرة
            st.download_button(
            ...
            st.info("💡 بمجرد الضغط على الزر الأخضر بالأعلى، سيقوم المتصفح بتحميل الملف فوراً وتجده داخل مجلد (التحميلات / Downloads) الرئيسي في هاتفك جاهزاً للنقل للكمبيوتر وتشغيله على الـ ArcMap المرجعي.")
