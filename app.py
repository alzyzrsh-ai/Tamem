...
step = st.sidebar.radio("اختر مرحلة العمل الحالية:", [
    ...
    "2️⃣ الاستدعائي التلقائي للبيانات والأقمار الصناعية",
...
# ==================== الخطوة الرابعة (تمت معالجة الخطأ هنا) ====================
elif step == "4️⃣ الفرز والتتبع الآلي للكسور والصدوع الجيولوجية":
    st.subheader("🚨 المرحلة الرابعة: تتبع متجهات خطوط الصدوع والكسور آلية بالكامل")
    ...
    if st.session_state['satellite_data'] is None:
        ...
        # حساب الحواف عبر مرشحات التدرج الرياضي المباشر
        ...
        for r in range(2, h-2, 3):
            for c in range(2, w-2, 3):
                if len(line_segments) > 250: 
                    break 
                if lineament_mask[r, c]: # تم تصحيح السطر هنا وتبسيطه
        ...
        if not df_lines.empty:
            ...
            st.info("💡 **نصيحة هندسية للـ ArcMap:** لفتح ملف الـ CSV كخطوط حقيقية فوراً، استخدم أداة **(XY To Line)** الموجودة داخل صندوق أدوات **Data Management Tools -> Features** ليرسم لك شبكة الصدوع آلياً فوق طبقاتك المفتوحة!")
        else:
