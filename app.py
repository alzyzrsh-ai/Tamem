...
# ==================== الخطوة الرابعة (تم تصحيح ترتيب الأكسل هنا) ====================
    ...
    if st.session_state['satellite_data'] is None:
        ...
        if not target_df.empty:
            # الحل السحري: إضافة sep=, لضمان ترتيب الأعمدة في إكسل فورا
            csv_string = "sep=,\n" + target_df.to_csv(index=False)
            csv_export = csv_string.encode('utf-8-sig') # استخدام utf-8-sig لدعم اللغة العربية دون رموز غريبة
...
# ==================== الخطوة الخامسة (تم تصحيح ترتيب الأكسل هنا) ====================
    ...
    if st.session_state['satellite_data'] is None:
        ...
        if not df_lines.empty:
            ...
            # الحل السحري: إضافة التوجيه لبرنامج إكسل لفصل الأعمدة بدقة
            csv_lines_str = "sep=,\n" + df_lines.to_csv(index=False)
            csv_lines = csv_lines_str.encode('utf-8-sig')
