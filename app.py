...
# ==========================================
    ...
    # ----------------------------------------------------
    # ج) معالجة الصورة عبر الذكاء الاصطناعي
    # ----------------------------------------------------
    ...
    if run_inversion:
        with st.spinner("جاري حل المعضلات العكسية ومطابقة المنحنى..."):
            # دالة تقريبية لنماذج الطبقات المترابطة (Forward Filter Approximation)
            def forward_schumberger(rho_layers, thick_layers, ab2_arr):
                # نموذج تقريبي لحساب المقاومية الظاهرية بناء على سمك ومقاومية الطبقات
                rho_calc = []
                for s in ab2_arr:
                    val = rho_layers[0]
                    cum_d = 0
            ...
            with c2:
                # رسم المنحنى الميداني والنظري معاً (IPI2Win Curve Fitting)
