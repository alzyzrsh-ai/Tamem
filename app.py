...
st.title("🛰️ المنصة الذكية المتكاملة للاستشعار عن بعد والفرز الطباقي والتحليل الحراري والكسور")
st.write("---")
...
# ==================== الخطوة الخامسة (محدثة بالكامل لتصدير الخريطة الراستر والـ TIF لـ ArcMap) ====================
    ...
    if st.session_state['satellite_data'] is None:
        ...
        # بناء مصفوفة راستر نقية (Raster Layer) لتصديرها كملف TIF للكسور فقط لـ ArcMap
        # القيمة 1 تمثل وجود كسر، والقيمة 0 تمثل خلفية فارغة
        ...
        # --- قسم التصدير لـ ArcMap بصيغة TIF الحصرية والمباشرة ---
        st.write("---")
        st.subheader("💾 🗺️ التصدير المباشر لـ ArcMap بصيغة الراستر الجغرافي TIF:")
        st.info("💡 هذه الميزة تقوم بتوليد خريطة تصنيفية خطية (Raster Classification) للكسور مسقطة جغرافياً WGS84، يمكنك إسقاطها فوراً في ArcGIS كطبقة مستقلة وتغيير لونها بدقة.")
        ...
        if user_tif_fracture_name.strip():
            ...
            # تمرير مصفوفة الكسور النقية لتوليد ملف GeoTIFF
            ...
            st.download_button(
                ...
                data=fracture_geotiff_bytes,
                file_name=final_fracture_tif_name,
                mime="image/tiff",
                key="download_fracture_tif_btn"
            )
            
        # البيانات الجدولية المساعدة لإكسل
        df_lines = pd.DataFrame(line_segments)
        st.write("---")
        st.subheader("📂 جدول متجهات خطوط الكسور والصدوع المستخرجة (اختياري للاستيراد كـ XY Data):")
