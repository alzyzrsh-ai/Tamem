...
# إعدادات الصفحة
st.set_page_config(page_title="الجاذبية الأرضية", layout="wide")

st.header("🛰️ استكشاف شذوذ الجاذبية الأرضية (Satellite Gravity)")
st.write("تحليل بيانات الشذوذ المتبقي لنموذج WGM2012 المعتمد على أقمار GRACE للتحري عن الأحواض الرسوبية والكسور.")
st.write("---")

# 1. جلب الإحداثيات المسجلة من الذاكرة المشتركة
if 'coords' in st.session_state:
    lat = st.session_state['coords'].get('lat', 16.270000)
    lon = st.session_state['coords'].get('lon', 44.270000)
    st.info(f"📍 موقع الدراسة المعتمد: Latitude ({lat}), Longitude ({lon})")
else:
...
# 2. تحديد نطاق المسح حول الموقع
buffer = st.slider("🌐 نطاق المسح الإقليمي حول الموقع (بالدرجات):", 0.1, 1.0, 0.2, step=0.05)

if st.button("🚀 جلب وتحليل شبكة الجاذبية"):
    with st.spinner("جاري الاتصال بقواعد البيانات وتحليل شذوذ الجاذبية..."):
        ...
        # محاكاة حساب شذوذ البوجيه المتبقي (Residual Bouguer Anomaly)
        ...
        # إظهار خطوط الكنتور
        lines = ax.contour(grid_lon, grid_lat, zi, levels=10, colors='black', alpha=0.4, linewidths=0.5)
        ax.clabel(lines, inline=True, fontsize=8)
        
        # إضافة موقع الجسة/الموقع الميداني
        ax.scatter(lon, lat, color='yellow', edgecolor='black', s=120, marker='*', label='موقع الدراسة')
        
        ax.set_title("Residual Gravity Anomaly Map (WGM2012 Model)", fontsize=12)
        ax.set_xlabel("Longitude (°E)")
        ax.set_ylabel("Latitude (°N)")
        ax.legend(loc='upper right')
        
        st.pyplot(fig)
        st.success("تم التراكب وتحليل شذوذ الجاذبية بنجاح!")
